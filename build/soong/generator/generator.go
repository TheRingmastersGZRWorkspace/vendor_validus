...
func init() {
	android.RegisterModuleType("validus_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = validusExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = validusExpandVariables(ctx, dep_file)
	...
	cmd := validusExpandVariables(ctx, String(g.properties.Cmd))
