function __print_validus_functions_help() {
cat <<EOF
Additional validus functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- validusgerrit:   A Git wrapper that fetches/pushes patch from/to validusRoms Gerrit Review.
- validusrebase:   Rebase a Gerrit change and push it again.
- validusremote:   Add git remote for validusRoms Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for validusRoms Github.
...
- validus:        Sets up build environment using breakfast(),
                   and then compiles using mka() against validus target.
EOF
}
...
function validusremote()
{
    ...
    local VALIDUS_USER=$(git config --get review.review.validusroms.com.username)
    if [ -z "$VALIDUS_USER" ]
    then
        git remote add validus ssh://review.validusroms.com:29418/$PFX$PROJECT
    else
        git remote add validus ssh://$VALIDUS_USER@review.validusroms.com:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/ValidusOS/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        validusremote
        git push validus HEAD:refs/heads/'$1'
    fi
    '
}

function validusgerrit() {
    ...
    local user=`git config --get review.review.validusroms.com.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "validusgerrit" ]; then
...
function validusrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "VALIDUSROMS Gerrit Rebase Usage: "
        echo "      validusrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.validusroms.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
